<?PHP
    ...
    $content = <<< EOT
        ...
        <form method="post"action="/inc/php/manageDBsrc.php">
            Select table which column to delete:
            <select name = selectedTable>
                <option value="">Select Table</option>
                {$options}
            </select><br>
            <input type="text"name="selectedColumn"placeholder="Column name">
            <input type="submit"name="deleteColumn"value="Delete Column"onclick="return confirm('Are you sure you want to delete the column?')">
        </form>
EOT;
